...
int main(){
    ...
    for(int i = 1; i < n; ++i){
        ...
        while(x != old_scc){
            x = graph[x].parent;
            old_scc = graph[x].scc_parent;
        }
        graph[x1].scc_parent = x1;
        long long children_old = 0, children_new = 0;
        stack<int> st1, st2;
        st1.push(old_scc);
        st2.push(x1);
        while(!st1.empty() && !st2.empty()){
            int v1 = st1.top(); st1.pop();
            int v2 = st2.top(); st2.pop();

            for(auto& c: graph[v1]){
                if(c != graph[v1].parent){
                    children_old++; 
                    graph[c].parent = v1;
                    graph[c].scc_parent = old_scc;
                    st1.push(c);
                }
            }

            for(auto& c: graph[v2]){
                if(c != graph[v2].parent){
                    children_new++; 
                    graph[c].parent = v2;
                    graph[c].scc_parent = x1;
                    st2.push(c);
                }
            }
        }
        if(st1.empty() && st2.empty()){
            graph[graph[x1].scc_parent].children = children_old;
            graph[x1].children = children_new;
        }
        else if (!st1.empty() && st2.empty()){
            graph[old_scc].children -= children_new + 1;
            graph[x1].children = children_new;
        } else {
            graph[x1].children = graph[old_scc].children;
            graph[x1].children -= children_old + 1;
            graph[old_scc].children = children_old;
        }
        cout << graph[x1].children + 1 << endl;
        if(i + 1 < n){
            x1 = st.query((graph[x1].children + 1 + g[i - 1]) % (n - i - 1) + 1);
        }
    }
}