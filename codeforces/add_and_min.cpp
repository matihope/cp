...
struct segtree{
    ...
    void add(int l, int r, int val, int x, int tl, int tr){
        ...
        if(l == tl && r == tr){
            ...
            add(max(l, m + 1), r, val, x * 2 + 1, m + 1, tr);
            mini[x] = min(mini[x * 2], mini[x * 2 + 1]) + t[x];
        }
    }
    ...
    LL get(int l, int r, int x, int tl, int tr){
        ...
        int m = (tl + tr) / 2;
        return min(get(l, min(r, m), x * 2, tl, m), get(max(l, m + 1), r, x * 2 + 1, m + 1, tr)) + t[x];
    }
};
int main()
{
