...
struct segtree{
    ...
    void set(int vert, int x, int tl, int tr){
        if(tl == tr)
            t[x] = 1;
        else{
            int m = (tl + tr) / 2;
    ...
    LL query(int l, int r, int x, int tl, int tr){
        ...
        int m = (tl + tr) / 2;
        return query(l, min(m, r), x * 2, tl, m) + query(max(l, m + 1), r, x * 2 + 1, m + 1, tr);
    }
...
int main()
{
    ...
    REP(i, n * 2){
        ...
        if(first_time[a] == -1){
            ...
            LL res = st.query(first_time[a], i);
            st.set(first_time[a]);
            answ[a - 1] = res;
        }
    }
