...
struct segtree{
    ...
    void set(int l, int r, int val){
        set(l, val, 1, 0, size - 1);
        set(r, -val, 1, 0, size - 1);
    }

    LL sum(int l, int r, int x, int tl, int tr){
        if(l > r) return 0;
        if(l == tl && r == tr){
            return t[x];
        } else {
            int m = (tl + tr) / 2;
            return sum(l, min(m, r), x * 2, tl, m) + sum(max(m + 1, l), r, x * 2 + 1, m + 1, tr);
        }
    }

    LL sum(int vertex){
        return sum(0, vertex, 1, 0, size - 1);
    }
